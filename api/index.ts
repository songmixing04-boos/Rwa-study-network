...
(function(){
  ...
  window.fetch=function(input,opts){
    ...
    if(_isBlocked(url)){var pu=_proxyUrl(url);return _origFetch(pu,opts);}
...
.rwa-social-btn{display:flex;align-items:center;gap:12px;width:100%;padding:11px 16px;border-radius:10px;text-decoration:none;font-size:14px;font-weight:600;margin-bottom:10px;transition:transform .15s,box-shadow .15s;border:none;cursor:pointer;}
...
#rwa-close-btn{position:absolute;top:10px;right:12px;background:none;border:none;color:#888;font-size:20px;cursor:pointer;line-height:1;padding:2px 6px;border-radius:6px;transition:color .15s,background .15s;}
...
(function(){
  ...
  overlay.addEventListener('click',function(e){
    if(e.target===overlay){overlay.classList.add('rwa-hidden');try{localStorage.setItem(KEY,Date.now().toString());}catch(e){}}
  });
})();
</script>`;
...
function rewriteHtml(html: string): string {
  ...
  html = html.replace(
    new RegExp(`(href|src|action|data-src|data-href|data-url|poster|srcset)=(['"])(https?:)?//rwa\\.studybeepro\\.in`, "gi"),
    "$1=$2/proxy"
  );
  html = html.replace(
  ...
  // Ticker
  const TICKER = `
<div id="rwa-ticker-wrap" style="width:100%;overflow:hidden;background:linear-gradient(90deg,#0a0a0a 0%,#1a1200 50%,#0a0a0a 100%);border-top:1px solid #FACC1540;border-bottom:1px solid #FACC1540;padding:5px 0;position:relative;z-index:999;">
<style>
@keyframes rwa-scroll{0%{transform:translateX(100vw)}100%{transform:translateX(-100%)}}
#rwa-ticker-inner{display:inline-block;white-space:nowrap;animation:rwa-scroll 28s linear infinite;font-family:'Syne','Inter',sans-serif;font-size:13px;font-weight:700;letter-spacing:.06em;}
...
<div id="rwa-ticker-inner">
  <span class="t1">🌟 RWA Study Network</span><span class="sep">✦</span>
  <span class="t2">Development by 🌺⃞⃪꯭𝓐𝓷𝓴𝓲𝓽 𝓒𝓱𝓪𝓾𝓭𝓱𝓪𝓻𝔂🦅</span><span class="sep">✦</span>
  <span class="t1">🌟 RWA Study Network</span><span class="sep">✦</span>
  <span class="t2">Development by 🌺⃞⃪꯭𝓐𝓷𝓴𝓲𝓽 𝓒𝓱𝓪𝓾𝓭𝓱𝓪𝓻𝔂🦅</span><span class="sep">✦</span>
</div>
</div>`;
...
function rewriteJs(js: string): string {
  js = js.replace(new RegExp(`(['"\`])https?://rwa\\.studybeepro\\.in`, "g"), "$1/proxy");
  return js;
}
...
app.options("/api/extproxy", (_req, res) => {
...
app.all("/api/extproxy", async (req, res) => {
  ...
  for (const h of ["auth-key","authorization","user-id","source","client-service","x-kgs-token","x-kgs-short","timestamp","content-type"]) {
  ...
  if (req.method !== "GET" && req.method !== "HEAD") {
    ...
    else if (ct.includes("application/x-www-form-urlencoded") && req.body) body = new URLSearchParams(req.body as Record<string,string>).toString();
  }
  try {
    const response = await fetch(targetUrl, { method: req.method === "OPTIONS" ? "GET" : req.method, headers: forwardHeaders, body, signal: AbortSignal.timeout(20000) } as RequestInit);
    ...
    if (contentType.includes("application/json")) {
      ...
      res.json(rewriteConfigJson(json));
    } else {
...
// Video worker proxy
app.options("/api/video-worker/*", (_req, res) => {
  res.setHeader("Access-Control-Allow-Origin", "*");
...
app.all("/api/video-worker/*", async (req, res) => {
  ...
  const forwardHeaders: Record<string, string> = {
    ...
    "Accept-Language": "hi-IN,hi;q=0.9,en;q=0.8",
    Referer: SITE_ORIGIN + "/player",
    Origin: SITE_ORIGIN,
  };
  if (req.headers["cookie"]) forwardHeaders["Cookie"] = req.headers["cookie"] as string;
  try {
    const response = await fetch(targetUrl, { method: req.method, headers: forwardHeaders, signal: AbortSignal.timeout(20000) } as RequestInit);
    const contentType = response.headers.get("content-type") || "";
    res.setHeader("Access-Control-Allow-Origin", "*");
    res.setHeader("Access-Control-Allow-Methods", "GET, POST, OPTIONS");
    ...
    if (!res.headersSent) res.status(502).json({ error: "Video worker unreachable" });
  }
});
...
app.all("/proxy/*", async (req, res) => {
  ...
  const forwardHeaders: Record<string, string> = {
    "User-Agent": "Mozilla/5.0 (Linux; Android 10; Mobile) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36",
    Accept: (req.headers["accept"] as string) || "*/*",
    "Accept-Language": (req.headers["accept-language"] as string) || "hi-IN,hi;q=0.9,en;q=0.8",
  ...
  if (method !== "GET" && method !== "HEAD" && method !== "OPTIONS") {
    const ct = (req.headers["content-type"] as string) || "";
    if (ct.includes("application/json") && req.body) { bodyPayload = JSON.stringify(req.body); forwardHeaders["Content-Type"] = "application/json"; }
    else if (ct.includes("application/x-www-form-urlencoded") && req.body) { bodyPayload = new URLSearchParams(req.body as Record<string,string>).toString(); forwardHeaders["Content-Type"] = "application/x-www-form-urlencoded"; }
  }

  try {
    const response = await fetch(targetUrl, { method, headers: forwardHeaders, body: bodyPayload, redirect: "follow", signal: AbortSignal.timeout(30000) } as RequestInit);
    const contentType = response.headers.get("content-type") || "";

    const rawSetCookie = response.headers.get("set-cookie");
    if (rawSetCookie) {
      const cleaned = rawSetCookie.replace(/;\s*domain=[^;,]*/gi, "").replace(/;\s*samesite=(?:strict|lax)/gi, "; SameSite=None").replace(/;\s*secure/gi, "");
    ...
    if (contentType.includes("text/html")) {
      res.send(rewriteHtml(await response.text()));
    } else if (contentType.includes("text/css")) {
      res.send(rewriteCss(await response.text()));
    } else if (contentType.includes("javascript") || contentType.includes("ecmascript")) {
      res.send(rewriteJs(await response.text()));
    } else {
