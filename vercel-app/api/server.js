...
app.get("/proxy/rwa-network-logo.svg", (_req, res) => {
  res.type("image/svg+xml").send(readFileSync(new URL("../public/rwa-network-logo.svg", import.meta.url), "utf8"));
});
...
<div id="rwa-popup-overlay">
  <div id="rwa-popup-box">
    ...
    <a class="rwa-social-btn rwa-btn-tg" href="https://t.me/+DS3-I_v2-6UzN2I1" target="_blank" rel="noopener">
    ...
    <a class="rwa-social-btn rwa-btn-wa" href="https://whatsapp.com/channel/0029VaLk0BK17En2Kcjwcc2r" target="_blank" rel="noopener">
...
function rewriteCss(css) {
  css = css.replace(new RegExp(`url\\((['"]?)https?://rwa\\.streamfiles\\.eu\\.org`, "g"), "url($1/proxy");
...
app.all("/proxy/*", async (req, res) => {
  ...
  try {
    ...
    if (targetUrl.endsWith("/api/app-config.php") && ct.includes("application/json")) {
    ...
    } else if (ct.includes("text/html")) res.send(rewriteHtml(await response.text()));
